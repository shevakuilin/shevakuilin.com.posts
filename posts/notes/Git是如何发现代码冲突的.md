...
**Myers 算法** 是一个差异算法，出自尤金·迈尔斯（Eugene W. Myers）之手，是一个能在大部分情况产生**最直观最简短** diff 的一个算法，也是一个典型的「**动态规划**」算法，父问题的最优解归结为子问题的最优解，也就是「**最优子结构**」。要知道 A1 的最优坐标，必须先要知道 A2 的最优坐标，要知道 A2  的最优坐标，必须知道 A3 的结果，以此类推。
...
**Myers算法**就是提供了找到最短路径的一种策略，其会通过**广度优先搜索**（BFS）图表中每条可能的路线，并在到达最终位置时立即停止。不过仅以 BFS 是无法计算出具体的这些编辑内容的，它仅能计算出我们需要进行的最少编辑次数，换句话说就是，BFS 只能找到发生改动的位置，而不是计算出改动的内容是什么。所以我们需要一个回溯算法来回溯改动内容的历史记录，通过图搜索，我们可以探索所有可能的编辑路径，直到找到到达终点所需的最小路径。到达终点后，我们可以进行回溯，来反向查看记录的数据，以找出导致结果的单个路径。所以，一个完整的**Myers算法**算法由三部分组成：

- 找到最短路径，以将`a`转换为`b`
- 找到所需的最少编辑量
- 通过回溯，找出导致结果的单个路径，也就是具体改动内容

该算法的最小版本可计算出我们需要进行的最少编辑次数，但不会计算出这些编辑的内容。为此，我们需要追溯算法生成的*x*值的历史记录。

然而标准的 **Myers 算法**有一个很大的缺点，虽然其速度很快，但是空间消耗很大，如果输入文件比较大，其空间开销是不能接受的。所以，Git 真正用的是标准 **Myers 算法** 的一个**变体**，这个变体需要的空间开销要小得多。

想要了解具体的 Myers 算法的实现，可以参考[他的论文](http://www.xmailserver.org/diff2.pdf)。
